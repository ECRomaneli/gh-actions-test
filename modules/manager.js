const { Tray, Menu, screen, dialog, globalShortcut, ipcMain, BrowserWindow, clipboard } = require('electron')
...
class Manager {
    ...
    refreshContextMenu() {
        ...
        menuItems.push({ id: 'close-all-windows', label: 'Close All Windows', click: () => {
            ...
            this.fromClipboardPage.window?.close()
        } })
        ...
        this.contextMenuListener = () => {
            let windows = this.pages.filter(p => p.window).length
            this.fromClipboardPage.window && windows++
